...
public class DisplayImageMetaData {
    ...
    public static String displayImageInfo(ImageInfo info) {
        String dataInfo;
        try {
            //info.setVerbose(1);
            dataInfo = "Info PreviewType is " + previewTypeAsString(info.getPreviewType())
            +"Info Monochrome is " + info.getMonochrome()
            +"Info Colorspace is " + colorspaceTypeAsString(info.getColorspace())
            +"Info Resolution units is " + info.getUnits()
            +"Info Compression is " + compressionTypeAsString(info.getCompression())
            +"Info Density is " + info.getDensity()
            +"Info magick is " + info.getMagick()
            +"Info filename is " + info.getFileName();
        }
        catch (Exception ex) {
            ex.printStackTrace();
            dataInfo = "Error";
