...
/**
     ...
     * @throws MagickException on error
     */
    public native MagickImage gaussianBlurImage(double radius, double sigma)
	throws MagickException;
   ...
   /**
     ...
     *
     * Defaults:
     * iterations: 1, kernelArgs: 1
     *
     * @param morphologyMethod the morphology method to use
     * @param kernelType the type of kernel to use
     * @return the image that has been operated on
     * @throws MagickException on error
     */
    public MagickImage morphologyImage(int morphologyMethod, int kernelType) throws MagickException {
        // defaults
        int iterations = 1;
        int kernelArgs = 1;
        return this.morphologyImage(morphologyMethod, iterations, kernelType, kernelArgs);
    }
     ...
     * @param morphologyMethod the morphology method to use
     * @param kernelType the type of kernel to use
     * @param kernelArgs pass the number for kernel args
     * @return the image that has been operated on
     * @throws MagickException on error
     */
    public MagickImage morphologyImage(int morphologyMethod, int kernelType, int kernelArgs) throws MagickException {
        // defaults
        int iterations = 1;
        return this.morphologyImage(morphologyMethod, iterations, kernelType, kernelArgs);
    }
     ...
     *
     * @param morphologyMethod the morphology method to use
     * @param iterations how many times to apply the operation
     * @param kernelType the type of kernel to use
     * @param kernelArgs pass the number for kernel args
     * @return the image that has been operated on
     * @throws MagickException on error
     */
    public MagickImage morphologyImage(int morphologyMethod, int iterations, int kernelType, int kernelArgs) throws MagickException {
        String[] kernels = {
            "Undefined", "Unity", "Gaussian",
            "DoG", "LoG", "Blur",
            "Comet", "Binomial", "Laplacian",
            "Sobel", "FreiChen", "Roberts",
            "Prewitt", "Compass", "Kirsch",
            "Diamond", "Square", "Rectangle",
            "Octagon", "Disk", "Plus",
            "Cross", "Ring", "Peaks",
            "Edges", "Corners", "Diagonals",
            "LineEnds", "LineJunctions", "Ridges",
            "ConvexHull", "ThinSE", "Skeleton",
            "Chebyshev", "Manhattan", "Octagonal",
            "Euclidean", "UserDefined"
        };
    
        HashMap<Integer, String> kernelTypeMap = new HashMap<Integer, String>();
        for (int i = 0; i < kernels.length; i++) {
            kernelTypeMap.put(i, kernels[i]);
        }

        // get the kernel string for the kernelType ID the user specified
        String kernel = kernelTypeMap.get(kernelType);
        String kernelString = String.format("%s:%d", kernel, kernelArgs);

        return this.morphologyImageInternal(morphologyMethod, iterations, kernelString);
    }

    /**
     * MorphologyImageInternal() applies a user supplied kernel to the image according to
     * the given mophology method.
     *
     * Generally you will never need to use this internal method,
     * but if you want to do advanced settings, this will allow you to
     *
     * @param morphologyMethod the morphology method to use
     * @param iterations how many times to apply the operation
     * @param kernelString a string representing the kernel type and kernel args (or custom string)
     * @return the image that has been operated on
     * @throws MagickException on error
     */
    public native MagickImage morphologyImageInternal(int morphologyMethod, int iterations, String kernelString)
        throws MagickException;
